...
class Pancreas(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        # aa = self._base_dir + "/Pancreas_h5/" + image_name + "_norm.h5"
        h5f = h5py.File(self._base_dir + "/Pancreas_h5/" + image_name + "/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
...
class Pancreas_no_read(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        for i in range(len(self.image_list)):
            ...
            # self.image_list_data.append(
            #     h5py.File(self._base_dir + "/Pancreas_h5/" + self.image_list[i] + "/mri_norm2.h5", 'r'))
            # demo = self.image_list_data[0]['image'][:]
            # print('fsd')
